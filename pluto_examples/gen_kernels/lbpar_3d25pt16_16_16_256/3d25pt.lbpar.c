...
#pragma omp parallel for private(lbv,ubv,t3,t4,t5,t6,t7,t8)
    for (t2=lbp;t2<=ubp;t2++) {
      for (t3=max(ceild(16*t2-Nz-3,16),4*t1-4*t2);t3<=min(min(floord(4*Nt+Ny-25,16),floord(16*t2+Ny+3,16)),floord(64*t1-64*t2+Ny+55,16));t3++) {
        for (t4=max(max(ceild(t1-t2-3,4),ceild(16*t2-Nz-243,256)),ceild(16*t3-Ny-243,256));t4<=min(min(min(floord(4*Nt+Nx-25,256),floord(16*t2+Nx+3,256)),floord(16*t3+Nx+3,256)),floord(64*t1-64*t2+Nx+55,256));t4++) {
          for (t5=max(max(max(ceild(16*t2-Nz+5,4),ceild(16*t3-Ny+5,4)),ceild(256*t4-Nx+5,4)),16*t1-16*t2);t5<=min(min(min(min(Nt-5,4*t2+2),4*t3+2),64*t4+62),16*t1-16*t2+15);t5++) {
