...
#pragma omp parallel for private(lbv,ubv,t3,t4,t5,t6,t7,t8)
    for (t2=lbp;t2<=ubp;t2++) {
      for (t3=max(ceild(8*t2-Nz+5,8),4*t1-4*t2);t3<=min(min(floord(4*Nt+Ny-25,8),floord(8*t2+Ny-5,8)),floord(32*t1-32*t2+Ny+23,8));t3++) {
        for (t4=max(max(ceild(t1-t2-63,64),ceild(8*t2-Nz-2035,2048)),ceild(8*t3-Ny-2035,2048));t4<=min(min(min(floord(4*Nt+Nx-25,2048),floord(8*t2+Nx-5,2048)),floord(8*t3+Nx-5,2048)),floord(32*t1-32*t2+Nx+23,2048));t4++) {
          for (t5=max(max(max(ceild(8*t2-Nz+5,4),ceild(8*t3-Ny+5,4)),ceild(2048*t4-Nx+5,4)),8*t1-8*t2);t5<=min(min(min(min(2*t2,2*t3),Nt-5),512*t4+510),8*t1-8*t2+7);t5++) {
