...
#pragma omp parallel for private(lbv,ubv,t3,t4,t5,t6,t7,t8)
    for (t2=lbp;t2<=ubp;t2++) {
      for (t3=max(ceild(4*t1-4*t2-2,3),ceild(8*t2-Nz-11,24));t3<=min(min(floord(4*Nt+Ny-25,24),floord(8*t2+Ny-5,24)),floord(32*t1-32*t2+Ny+23,24));t3++) {
        for (t4=max(max(ceild(t1-t2-7,8),ceild(8*t2-Nz-243,256)),ceild(24*t3-Ny-243,256));t4<=min(min(min(floord(4*Nt+Nx-25,256),floord(8*t2+Nx-5,256)),floord(24*t3+Nx+11,256)),floord(32*t1-32*t2+Nx+23,256));t4++) {
          for (t5=max(max(max(ceild(8*t2-Nz+5,4),ceild(24*t3-Ny+5,4)),ceild(256*t4-Nx+5,4)),8*t1-8*t2);t5<=min(min(min(min(2*t2,Nt-5),6*t3+4),64*t4+62),8*t1-8*t2+7);t5++) {
