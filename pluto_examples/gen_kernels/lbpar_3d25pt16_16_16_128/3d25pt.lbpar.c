...
#pragma omp parallel for private(lbv,ubv,t3,t4,t5,t6,t7,t8)
    for (t2=lbp;t2<=ubp;t2++) {
      for (t3=max(ceild(16*t2-Nz-3,16),4*t1-4*t2);t3<=min(min(floord(4*Nt+Ny-25,16),floord(16*t2+Ny+3,16)),floord(64*t1-64*t2+Ny+55,16));t3++) {
        for (t4=max(max(ceild(t1-t2-1,2),ceild(16*t2-Nz-115,128)),ceild(16*t3-Ny-115,128));t4<=min(min(min(floord(4*Nt+Nx-25,128),floord(16*t2+Nx+3,128)),floord(16*t3+Nx+3,128)),floord(64*t1-64*t2+Nx+55,128));t4++) {
          for (t5=max(max(max(ceild(16*t2-Nz+5,4),ceild(16*t3-Ny+5,4)),ceild(128*t4-Nx+5,4)),16*t1-16*t2);t5<=min(min(min(min(Nt-5,4*t2+2),4*t3+2),32*t4+30),16*t1-16*t2+15);t5++) {
