...
#pragma omp parallel for private(lbv,ubv,t3,t4,t5,t6,t7,t8)
    for (t2=lbp;t2<=ubp;t2++) {
      for (t3=max(ceild(8*t2-Nz-19,32),t1-t2);t3<=min(min(floord(4*Nt+Ny-25,32),floord(8*t2+Ny-5,32)),floord(32*t1-32*t2+Ny+23,32));t3++) {
        for (t4=max(max(ceild(t1-t2-1,2),ceild(8*t2-Nz-51,64)),ceild(32*t3-Ny-51,64));t4<=min(min(min(floord(4*Nt+Nx-25,64),floord(8*t2+Nx-5,64)),floord(32*t3+Nx+19,64)),floord(32*t1-32*t2+Nx+23,64));t4++) {
          for (t5=max(max(max(ceild(8*t2-Nz+5,4),ceild(32*t3-Ny+5,4)),ceild(64*t4-Nx+5,4)),8*t1-8*t2);t5<=min(min(min(min(2*t2,Nt-5),8*t3+6),16*t4+14),8*t1-8*t2+7);t5++) {
