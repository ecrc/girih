...
#pragma omp parallel for private(lbv,ubv,t3,t4,t5,t6,t7,t8)
    for (t2=lbp;t2<=ubp;t2++) {
      for (t3=max(ceild(8*t2-Nz-3,16),2*t1-2*t2);t3<=min(min(floord(4*Nt+Ny-25,16),floord(8*t2+Ny-5,16)),floord(32*t1-32*t2+Ny+23,16));t3++) {
        for (t4=max(max(ceild(t1-t2-15,16),ceild(8*t2-Nz-499,512)),ceild(16*t3-Ny-499,512));t4<=min(min(min(floord(4*Nt+Nx-25,512),floord(8*t2+Nx-5,512)),floord(16*t3+Nx+3,512)),floord(32*t1-32*t2+Nx+23,512));t4++) {
          for (t5=max(max(max(ceild(8*t2-Nz+5,4),ceild(16*t3-Ny+5,4)),ceild(512*t4-Nx+5,4)),8*t1-8*t2);t5<=min(min(min(min(2*t2,Nt-5),4*t3+2),128*t4+126),8*t1-8*t2+7);t5++) {
