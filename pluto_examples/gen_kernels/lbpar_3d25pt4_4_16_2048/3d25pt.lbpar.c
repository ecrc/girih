...
#pragma omp parallel for private(lbv,ubv,t3,t4,t5,t6,t7,t8)
    for (t2=lbp;t2<=ubp;t2++) {
      for (t3=max(ceild(4*t2-Nz-3,16),t1-t2);t3<=min(min(floord(4*Nt+Ny-25,16),floord(4*t2+Ny-9,16)),floord(16*t1-16*t2+Ny+7,16));t3++) {
        for (t4=max(max(ceild(t1-t2-127,128),ceild(4*t2-Nz-2035,2048)),ceild(16*t3-Ny-2035,2048));t4<=min(min(min(floord(4*Nt+Nx-25,2048),floord(4*t2+Nx-9,2048)),floord(16*t3+Nx+3,2048)),floord(16*t1-16*t2+Nx+7,2048));t4++) {
          for (t5=max(max(max(ceild(4*t2-Nz+5,4),ceild(16*t3-Ny+5,4)),ceild(2048*t4-Nx+5,4)),4*t1-4*t2);t5<=min(min(min(min(Nt-5,t2-1),4*t3+2),512*t4+510),4*t1-4*t2+3);t5++) {
