...
#pragma omp parallel for private(lbv,ubv,t3,t4,t5,t6,t7,t8)
    for (t2=lbp;t2<=ubp;t2++) {
      for (t3=max(ceild(t1-t2-1,2),ceild(4*t2-Nz-19,32));t3<=min(min(floord(4*Nt+Ny-25,32),floord(4*t2+Ny-9,32)),floord(16*t1-16*t2+Ny+7,32));t3++) {
        for (t4=max(max(ceild(t1-t2-63,64),ceild(4*t2-Nz-1011,1024)),ceild(32*t3-Ny-1011,1024));t4<=min(min(min(floord(4*Nt+Nx-25,1024),floord(4*t2+Nx-9,1024)),floord(32*t3+Nx+19,1024)),floord(16*t1-16*t2+Nx+7,1024));t4++) {
          for (t5=max(max(max(ceild(4*t2-Nz+5,4),ceild(32*t3-Ny+5,4)),ceild(1024*t4-Nx+5,4)),4*t1-4*t2);t5<=min(min(min(min(Nt-5,t2-1),8*t3+6),256*t4+254),4*t1-4*t2+3);t5++) {
