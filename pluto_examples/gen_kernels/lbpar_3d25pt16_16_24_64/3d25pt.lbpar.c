...
#pragma omp parallel for private(lbv,ubv,t3,t4,t5,t6,t7,t8)
    for (t2=lbp;t2<=ubp;t2++) {
      for (t3=max(ceild(8*t1-8*t2-2,3),ceild(16*t2-Nz-11,24));t3<=min(min(floord(4*Nt+Ny-25,24),floord(16*t2+Ny+3,24)),floord(64*t1-64*t2+Ny+55,24));t3++) {
        for (t4=max(max(ceild(16*t2-Nz-51,64),ceild(24*t3-Ny-51,64)),t1-t2);t4<=min(min(min(floord(4*Nt+Nx-25,64),floord(16*t2+Nx+3,64)),floord(24*t3+Nx+11,64)),floord(64*t1-64*t2+Nx+55,64));t4++) {
          for (t5=max(max(max(ceild(16*t2-Nz+5,4),ceild(24*t3-Ny+5,4)),ceild(64*t4-Nx+5,4)),16*t1-16*t2);t5<=min(min(min(min(Nt-5,4*t2+2),6*t3+4),16*t4+14),16*t1-16*t2+15);t5++) {
            for (t6=max(16*t2,4*t5+4);t6<=min(16*t2+15,4*t5+Nz-5);t6++) {
              for (t7=max(24*t3,4*t5+4);t7<=min(24*t3+23,4*t5+Ny-5);t7++) {
                for (t8=max(64*t4,4*t5+4);t8<=min(64*t4+63,4*t5+Nx-5);t8++) {
