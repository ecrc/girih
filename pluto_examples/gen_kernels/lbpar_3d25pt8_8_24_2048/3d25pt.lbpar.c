...
if ((Nt >= 5) && (Nx >= 9) && (Ny >= 9) && (Nz >= 9)) {
  for (t1=0;t1<=floord(5*Nt+Nz-30,8);t1++) {
    lbp=max(ceild(4*t1,5),ceild(8*t1-Nt+5,8));
    ubp=min(min(floord(4*Nt+Nz-25,8),floord(32*t1+Nz+23,40)),t1);
#pragma omp parallel for private(lbv,ubv,t3,t4,t5,t6,t7,t8)
    for (t2=lbp;t2<=ubp;t2++) {
      for (t3=max(ceild(4*t1-4*t2-2,3),ceild(8*t2-Nz-11,24));t3<=min(min(floord(4*Nt+Ny-25,24),floord(8*t2+Ny-5,24)),floord(32*t1-32*t2+Ny+23,24));t3++) {
        for (t4=max(max(ceild(t1-t2-63,64),ceild(8*t2-Nz-2035,2048)),ceild(24*t3-Ny-2035,2048));t4<=min(min(min(floord(4*Nt+Nx-25,2048),floord(8*t2+Nx-5,2048)),floord(24*t3+Nx+11,2048)),floord(32*t1-32*t2+Nx+23,2048));t4++) {
          for (t5=max(max(max(ceild(8*t2-Nz+5,4),ceild(24*t3-Ny+5,4)),ceild(2048*t4-Nx+5,4)),8*t1-8*t2);t5<=min(min(min(min(2*t2,Nt-5),6*t3+4),512*t4+510),8*t1-8*t2+7);t5++) {
            for (t6=max(8*t2,4*t5+4);t6<=min(8*t2+7,4*t5+Nz-5);t6++) {
