...
#pragma omp parallel for private(lbv,ubv,t3,t4,t5,t6,t7,t8)
    for (t2=lbp;t2<=ubp;t2++) {
      for (t3=max(ceild(24*t2-Nz+9,4),24*t1-24*t2+1);t3<=min(min(floord(4*Nt+Ny-25,4),floord(24*t2+Ny+11,4)),floord(96*t1-96*t2+Ny+87,4));t3++) {
        for (t4=max(max(ceild(3*t1-3*t2-31,32),ceild(24*t2-Nz-1011,1024)),ceild(4*t3-Ny-1011,1024));t4<=min(min(min(floord(4*Nt+Nx-25,1024),floord(24*t2+Nx+11,1024)),floord(4*t3+Nx-9,1024)),floord(96*t1-96*t2+Nx+87,1024));t4++) {
          for (t5=max(max(max(ceild(24*t2-Nz+5,4),ceild(4*t3-Ny+5,4)),ceild(1024*t4-Nx+5,4)),24*t1-24*t2);t5<=min(min(min(min(Nt-5,6*t2+4),t3-1),256*t4+254),24*t1-24*t2+23);t5++) {
