...
#pragma omp parallel for private(lbv,ubv,t3,t4,t5,t6,t7,t8)
    for (t2=lbp;t2<=ubp;t2++) {
      for (t3=max(ceild(24*t2-Nz+5,8),12*t1-12*t2);t3<=min(min(floord(4*Nt+Ny-25,8),floord(24*t2+Ny+11,8)),floord(96*t1-96*t2+Ny+87,8));t3++) {
        for (t4=max(max(ceild(3*t1-3*t2-7,8),ceild(24*t2-Nz-243,256)),ceild(8*t3-Ny-243,256));t4<=min(min(min(floord(4*Nt+Nx-25,256),floord(24*t2+Nx+11,256)),floord(8*t3+Nx-5,256)),floord(96*t1-96*t2+Nx+87,256));t4++) {
          for (t5=max(max(max(ceild(24*t2-Nz+5,4),ceild(8*t3-Ny+5,4)),ceild(256*t4-Nx+5,4)),24*t1-24*t2);t5<=min(min(min(min(2*t3,Nt-5),6*t2+4),64*t4+62),24*t1-24*t2+23);t5++) {
