...
#pragma omp single
    ...
    for(t=tb; t< te-1; t++){
        ...
        if(t%2 == 1){
            ...
            output_buffer = p->U2;      //@KADIR
        }
        //SOURCE
        if( (p->source_point_enabled==1) 
            && (p->lsource_pt[0] >= p->stencil.r && p->lsource_pt[0] < p->lstencil_shape[0]+p->stencil.r) //@KADIR FIXME I think < instead of <= :-)
            && (p->lsource_pt[1] >= yb && p->lsource_pt[1] < ye) //@KADIR FIXME I think < instead of <= :-)
            && (p->lsource_pt[2] >= zb && p->lsource_pt[2] < ze) //@KADIR FIXME I think < instead of <= :-)
          ) 
        {
        ...
        printf("DIAsptB\tts:%d(%d-%d) idxU:%lu valU:%g src_exc_coef:%g "                //@KADIR
                "coef:%g %g %g %g %g\t"                                       
                "x(%d-%d) y(%d-%d)\tz(%d-%d)\n",
                t, tb, te, idxU, output_buffer[idxU], p->src_exc_coef[t], 
