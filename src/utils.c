...
void set_custom_receivers(Parameters *p) {//@KADIR receiver coordinates are taken from exawave.xml
  ...
  for(i=0; i<p->num_receivers; i++) {
    ...
    p->receiver_pt[i][2] = 100;//TODO 400;//(p->stencil_shape[2]+2*p->stencil.r)/2 -1 
      }
  }
}
...
void init_coeff(Parameters * p) {
  ...
  fclose(fp);
  real_t mval=fabs(p->src_exc_coef[0]);
  for (it=0; it<p->nt;it++) {
    if(mval < fabs(p->src_exc_coef[it])){
        mval = fabs(p->src_exc_coef[it]);
    }
  }
  if (mval<=0) mval=1;
  printf("\nmval:%g\n", mval);
