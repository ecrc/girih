...
def plot_lines(raw_data, stencil_kernel, is_dp):
    ...
    req_fields = [('Thread group size', int), ('Time stepper orig name', str), ('MPI size', int), ('MStencil/s MAX', float)]
    data = []
    for k in raw_data:
        tup = {}
    ...
    max_single = 0;
    for k in data:
        if ( ('Spatial Blocking' in k['Time stepper orig name']) and (k['stencil']==stencil_kernel) and (k['Precision']==is_dp)):
    ...
    perf_str = 'MStencil/s MAX'
    for tgs in [1,-1]:
        x = []
        y = []
        y_m = []
        for k in data:
            if ( ('Diamond' in k['Time stepper orig name']) and ((k['Thread group size'] == tgs) or (tgs==-1 and k['Thread group size']>1))  and (k['stencil']==stencil_kernel) and (k['Precision']==is_dp) ):
