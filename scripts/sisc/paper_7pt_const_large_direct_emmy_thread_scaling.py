...
"""export OMP_NUM_THREADS=$thn; likwid-perfctr -m -s 0x03 -g MEM -C S0:0-$th $exec_path --n-tests 2 --disable-source-point --npx 1 --npy 1 --npz 1 --nx 960 --ny 960 --nz 960  --verbose 1 --target-ts $ts --wavefront $wf --nt 100 --target-kernel $kernel --t-dim $tb --thread-group-size $tgs --num-wavefronts $nwf | tee $outfile""")
...
def main():
    ...
    exp_name = "emmy_socket_thread_scaling_7pt_const_960cube_f2.2GHz_mwd_swd_at_%s" % hostname  
    ...
    for th in list(range(10, 0, -2)):
        for is_dp in [1]:
            for ts, tgs, tb, nwf  in [(2, 2, 5, 12)]:  # DP
                for kernel in kl:
    ...
    for th in[5, 10]:
        for is_dp in [1]:
            for ts, tgs, tb, nwf  in [(2, 5, 11, 20)]:  #DP
                for kernel in kl:
                    outfile=(exp_name + '_ts%d_tb%d_tgs%d_nwf%d_kernel%d_isDP%d_threads%d.txt' % (ts, tb, tgs, nwf, kernel, is_dp, th))
                    submit_emmy_experiment(nwf=nwf, tgs=tgs, wf=wf, is_dp=is_dp, th=th, ts=ts, tb=tb, kernel=kernel, outfile=outfile, target_dir=target_dir)
    ...
    for th in[10]:
        for is_dp in [1]:
            for ts, tgs, tb, nwf  in [(2, 10, 11, 50)]:  #DP
                for kernel in kl:
                    outfile=(exp_name + '_ts%d_tb%d_tgs%d_nwf%d_kernel%d_isDP%d_threads%d.txt' % (ts, tb, tgs, nwf, kernel, is_dp, th))
                    submit_emmy_experiment(nwf=nwf, tgs=tgs, wf=wf, is_dp=is_dp, th=th, ts=ts, tb=tb, kernel=kernel, outfile=outfile, target_dir=target_dir)
    ...
    for th in list(range(10, 0, -1)):
        for is_dp in [1]:
            for ts, tgs, tb, nwf  in [(0, 0, 0, 0), (2, 1, 5, 1)]:
                for kernel in kl:
                    outfile=(exp_name + '_ts%d_tb%d_tgs%d_nwf%d_kernel%d_isDP%d_threads%d.txt' % (ts, tb, tgs, nwf, kernel, is_dp, th))
                    submit_emmy_experiment(nwf=nwf, tgs=tgs, wf=wf, is_dp=is_dp, th=th, ts=ts, tb=tb, kernel=kernel, outfile=outfile, target_dir=target_dir)
