...
#    lns = []
    ...
    # add power results at points of interest
    if t == 1:
        p_spt = '40W_26pJ/LUP'
        p_1wd = '36W_9pJ/LUP'
        p_2wd = '30W_8pJ/LUP'
...
#        plt.annotate(p_spt, xy=(10, y_m[-1]), xytext=(6.25, 35), 
#                     arrowprops=dict(facecolor='black', shrink=0.09, width=0.5, frac=0.2, headwidth=7))
...
#        plt.annotate(p_10wd, xy=(10, y_m[-1]), xytext=(7, 2.5), 
#                     arrowprops=dict(facecolor='black', shrink=0.09, width=0.5, frac=0.1, headwidth=7))


    cols = {0:'y', 1:'k', 2:'b', 4:'c', 5:'r', 8:'m'}
    markers = {0:'.', 1:'^', 2:'v', 4:'.', 5:'x', 8:'.'}
    ann_loc = {1:(6.25,0), 2:(6.25,3), 5:(7,-7)}
    if t==1: p_wd = {1:p_1wd, 2:p_2wd, 5:p_5wd}
    for tgs in [1,2, 4, 8, 5]:
        ...
        if(y_m) and t==1:
            plt.plot(x, y_m, color=col, marker=marker, linestyle='-', label=ts2)
#            plt.annotate(p_wd[tgs], xy=(10, y_m[-1]), xytext=(ann_loc[tgs][0], y_m[-1]+ann_loc[tgs][1]), 
#                     arrowprops=dict(facecolor='black', shrink=0.09, width=0.5, frac=0.1, headwidth=7))
...
#    sus_mem_bw = 36500 #SB
    ...
    if t == 0:
        title = '7_pt_const_all_methods_perf'
        plt.ylabel('GLUP/s')
    else:
        title = '7_pt_const_all_methods_bw'
        plt.ylabel('GBytes/s')
