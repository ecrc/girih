...
ibrun tacc_affinity $exec_path --n-tests 2 --disable-source-point --target-ts $ts --t-dim $t_dim --npx $npx --npy $npy --npz $npz --nx $nx --ny $ny --nz $nz --nt $nt --z-mpi-contig $is_contig
...
def main():
    ...
    exp_name = "stampede_tiny_strongscaling_halo_first"  
    ...
    for npz in [2, 4, 8, 16, 32]:
        outfile=(exp_name + '_ts%d_npz%d' % (k,npz))
