...
#SBATCH -n $nall                # Total number of mpi tasks requested
...
ibrun -o 0 -n $np tacc_affinity $exec_path --n-tests 2 --disable-source-point --target-ts $ts --t-dim $t_dim --npx $npx --npy $npy --npz $npz --nx $nx --ny $ny --nz $nz --nt $nt --z-mpi-contig $is_contig --halo-concatenate 1
...
""")
    ...
    ppn=2
    nn = int(ceil(1.0*np/ppn))
    nall = nn*ppn

    job_script = job_template.substitute(nall=nall, nn=nn, np=np, npx=npx, npy=npy, npz=npz, nx=nx, ny=ny, nz=nz, nt=nt, ts=ts, t_dim=t_dim, is_contig=is_contig, outfile=outpath, exec_path=exec_path, target_dir=target_dir, jobname=jobname)
...
def main():
    ...
    exp_name = "stampede_strongscaling_nb_naive_y_decomp_concat_1024cube"  
    ...
    #for npy in [16, 32]:
    #for npy in [64, 128]:
        outfile=(exp_name + '_ts%d_npy%d' % (k,npy))
