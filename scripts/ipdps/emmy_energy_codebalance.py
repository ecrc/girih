...
"""$exec_path --n-tests 2 --disable-source-point --npx 1 --npy 1 --npz 1 --nx $nx --ny $ny --nz $nz  --verbose 1 --target-ts $ts --nt $nt --target-kernel $kernel --cache-size $cs --thread-group-size $tgs --t-dim $Dw --num-wavefronts $nwf| tee $outfile""")
...
def ts_test(target_dir, exp_name, cs, ts, kernel, tgs, N, group): 
  is_dp = 1
  ...
  for Dw in [1,3,5,7]:
    ...
    outfile=(exp_name + 'isDP%d_ts%d_kernel%d_tgs%d_N%d_%s_TB%d.txt' % (is_dp, ts, kernel, tgs,  N, group,Dw))
    nx = N
    ny = N
    nz = N
...
def main():
  ...
  exp_name = "emmy_"
  exp_name = exp_name + "energy_codebalance_at_%s" % (hostname)  
  ...
  cs = 4096
  #ts_test(target_dir, exp_name[:-8], cs, ts=2, kernel=1, tgs=5, N=960, group='ENERGY') 
  ts_test(target_dir, exp_name[:-8], cs, ts=2, kernel=1, tgs=5, N=960, group='MEM') 

  #ts_test(target_dir, exp_name[:-8], cs, ts=2, kernel=5, tgs=5, N=480, group='ENERGY') 
  #ts_test(target_dir, exp_name[:-8], cs, ts=2, kernel=5, tgs=5, N=480, group='MEM') 
