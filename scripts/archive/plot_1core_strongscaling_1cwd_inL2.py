...
#    ('Sustained L2 BW', 2), ('Total L2 Transfer',2),
  ...
  data = sorted(data, key=itemgetter('Global NZ', 'LIKWID performance counter'))
  ...
  y_l3_bw = []
  for k in data:
    x.append(k['Global NZ'])
    y.append(k['MStencil/s  MAX'])
    y_l3_bw.append(k['Sustained L3 BW']/1024.0)


  lns = lns + ax1.plot(x, y, color='k', linestyle='-', label='Performance_DW4')

  proc_speed = 2.2*1e9
  ECM_inL2 =  proc_speed / ((17.7+10)/16) / ( 1024 * 1024)  
  lns = lns + ax1.plot([0, max(x)], [ECM_inL2, ECM_inL2], color='k', linestyle='--', label='ECM in L2 for naive')
  plt.ylim([0, 1300])

  ax2 = ax1.twinx()
  lns = lns + ax2.plot(x, y_l3_bw, color='r', marker='x', linestyle='-', label='obtained L2-L3 BW')

  ax1.set_xlabel('Size in Z')
  ax1.set_ylabel('MLUP/s')

  ax1.grid()
 
  title = 'In L2 problem_nx=512_ny=4_Ivy Bridge 1-core strongscaling, 7_pt const. stencil SP '
  plt.title(title)
