...
def main():
    ...
    plot_title='Strong scaling normalized'
    ...
    for p in range(len(procs)):
        for j in range(n_ts):
            ...
            if i < len(data):
                ...
                if j == 0:
                    ref_norm = comp_mean1[i] + comm_mean1[i]
                ...
                if kernels[j] == ts_list1[i]:
                    comp_mean.append(comp_mean1[i]/ref_norm)
                    comm_mean.append(comm_mean1[i]/ref_norm)
                    comp_max.append(comp_max1[i]/ref_norm)
                    comm_max.append(comm_max1[i]/ref_norm)
                    comp_min.append(comp_min1[i]/ref_norm)
                    comm_min.append(comm_min1[i]/ref_norm)   
                    i = i + 1
                else:
                    comp_mean.append(0)
                    comm_mean.append(0)
...
#    newax.tick_params('x', width=0)
    ...
    pylab.savefig('scaling_bars_normalized.png')
...
def plot_lines(raw_data, plot_title):
    ...
    plt.legend(loc='best')
    plt.grid()
    pylab.savefig('scaling_lines_normalized.png')
    #plt.show()    
