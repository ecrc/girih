...
#    ('Sustained L3 BW', 2), ('Total L3 Transfer',2),
  ...
  for k in data:
    ...
    y_l2_bw.append(k['Sustained L2 BW']/1024.0)


  lns = lns + ax1.plot(x, y, color='k', linestyle='-', label='Performance')

  ax2 = ax1.twinx()
  lns = lns + ax2.plot(x, y_l2_bw, color='r', marker='x', linestyle='-', label='obtained L1-L2 BW')

  ax1.set_xlabel('Size in X')
  ax1.set_ylabel('MLUP/s')

  ax1.grid()
 
  title = 'In L1 problem_nx=1_nz=1_Ivy Bridge 1-core strongscaling, 7_pt const. stencil SP '
  plt.title(title)
