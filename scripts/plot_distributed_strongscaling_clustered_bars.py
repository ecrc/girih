...
#         'lines.markersize': 3,
  ...
  req_fields = [('OpenMP Threads', int), ('Thread group size', int), ('Time stepper orig name', str), ('MPI size', int), ('MStencil/s MAX', float)]
...
# add MWD profiling
    measures = ['Wavefront barrier wait [s]:', 'Wavefront barrier wait [%]:',
               ...
               'Resolved diamonds:']
    if 'Diamond' in tup['Time stepper orig name']:
    ...
    measures =['RANK0 Total',
                ...
                'RANK0 Waiting',
                'RANK0 Other']
    if 'Spatial Blocking' in tup['Time stepper orig name']:
      for m in measures:
        tup[m] = float(k[m])
  ...
  for k in d2: # clean up the data and create the bars information
    ...
    k['idle_l'] = [0]
    k['others_l'] = [0]
    if 'Diamond' in k['Time stepper orig name']:
#      k['ts'] = str(k['Thread group size']) + 'WD'
      k['ts'] = '1WD' if(k['Thread group size']==1) else 'MWD'
      ...
      for i in range(k['ntg']):
        ...
        idle_l.append(k['Group spin-wait [%] group '+str(i)])
...
#  tsn_l = [k['Thread group size'] for k in data]
  ...
  for i in tsn_set: 
    if i == 0: ts_set.append('Spt.blk.')
      ...
      if(i==1): ts_set.append('1WD')
      else: ts_set.append('MWD')
  ...
  for i, k in enumerate(data):
    ...
    p1 = host.bar(idx, k['comp_mean'], width, color='c', align='center',yerr=[(k['comp_std'],),[0]], hatch="/", error_kw={'elinewidth':2, 'ecolor':ecolor,'capsize':3})
    p2 = host.bar(idx, k['comm_mean'], width, color='g', align='center',bottom=k['comm_btm'], yerr=[(k['comm_std'],),[0]], hatch="", error_kw={'elinewidth':2, 'ecolor':ecolor,'capsize':3})
    p3 = host.bar(idx, k['idle_mean'], width, color='r', align='center',bottom=k['idle_btm'], yerr=[(k['idle_std'],),[0]], hatch="\\", error_kw={'elinewidth':2, 'ecolor':ecolor,'capsize':3})
